...
def mostrar_ventana_registro(root):
    ...
    ventana_registro.title("Registro Docente")
    x_pos = root.winfo_x()
    y_pos = root.winfo_y()
    ventana_registro.geometry(f"{800}x{600}+{x_pos}+{y_pos}")

    # Cargar la imagen de fondo
    bg_image = Image.open("assets/FondoDocente.jpg")
    bg_image = bg_image.resize((800, 600), Image.LANCZOS)
    bg_photo = ImageTk.PhotoImage(bg_image)

    # Crear un widget Canvas
    canvas = tk.Canvas(ventana_registro, width=800, height=600)
    ...
    # input text loggin
    # nombre
    InputNombresReg = tk.Entry(ventana_registro, width=26)
    InputNombresReg.place(x=50, y=210)
    # apellidos
    InputApellidosReg = tk.Entry(ventana_registro, width=26)
    InputApellidosReg.place(x=50, y=310)
    # Documento registro unico
    InputDocumentoReg = tk.Entry(ventana_registro, width=26)
    InputDocumentoReg.place(x=50, y=410)

    imageBtnRegistrar = tk.PhotoImage(file="C:/Users/MiniMonster/Desktop/FaceToList/FaceToList/Assets/BtnRegistrar.png")
    boton_Registrar = tk.Button(ventana_registro, image=imageBtnRegistrar, command=Register)
    ...
    boton_cerrar = tk.Button(ventana_registro, image=imageBtnRegresar,
                             command=lambda: cerrar_ventana_registro(root, ventana_registro))
    boton_cerrar.place(x=550, y=500)


def cerrar_ventana_registro(root, ventana_registro):
...
def Register():
    global capture, lblVideo, pantalla2, InputNombresReg, InputApellidosReg, InputDocumentoReg, RegNombres, RegApellidos, RegDocumento
    #extract: name user pass
    RegNombres, RegApellidos, RegDocumento = InputNombresReg.get(), InputApellidosReg.get(), InputDocumentoReg.get()
    #incomplete form
    if len (RegNombres) == 0 or len(RegApellidos) == 0 or len(RegDocumento) == 0 :
